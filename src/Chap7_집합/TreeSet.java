...
class Sets {
	...
	{		
		...
		if(i==j) System.out.println("cycle"); //4,4는 사이클이다.
		WeightedUnion(i,j);
...
//		parent[j] = i;
	...
	// parent[i]=-count[i] and parent[j]=-count[j].
	{
		...
		if (parent[i] > parent[j]) { // i has fewer nodes // i.가 더 작아.
			parent[i] = j; //i가 j에 달라붙었다.
			parent[j] = temp;
