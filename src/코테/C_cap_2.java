...
public class C_cap_2 {
	 public static void main(String[] args) {
	        ...
	        int result = findMinManipulations(scores, k, m, cap);
	        System.out.println(result);
	    }
	    
	    private static int findMinManipulations(int[] scores, int k, int m, int cap) {
	        Arrays.sort(scores);
	        int scoreAbove = scores.length - m; // 본선 진출을 막기 위해 점수를 낮춰야 하는 인원 수
	        int manipulations = 0; // 조작 횟수
	        
	        for (int i = 0; i < scoreAbove; i++) {
	            if (scores[i] > k) {
	                manipulations += scores[i] - k;
	                scores[i] = k;
	            }
	        }
	        
	        Arrays.sort(scores); // 점수 조작 후 재정렬
	        
	        int equalScores = 0; // "나"와 동점인 사람들의 수
	        for (int score : scores) {
	            if (score == k) equalScores++;
	        }
	        
	        int spotsLeft = m - (equalScores - 1); // 본선 진출 가능한 남은 자리 수
	        if (spotsLeft >= 1) return manipulations; // 동점자 포함 본선 진출 가능
	        else return -1; // 본선 진출 불가능
	    }
}
