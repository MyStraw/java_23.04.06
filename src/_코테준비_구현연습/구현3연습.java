...
public class 구현3연습 {
	...
	public static void main(String args[]) {
		...
		//Watch(matrix); //시계방향 회전
		//ReverseWatch(matrix); //반시계방향 회전
		//upDown(matrix); // 상하반전
		//leftRight(matrix); // 좌우반전 (상하 + 좌우 하면 180이랑 같음)
		rotation180(matrix);// 180도 회전
	...
	private static void leftRight(int[][] matrix) {
	    ...
	    for (int i = 0; i < N; i++) {
	        for (int j = 0; j < N / 2; j++) {
	            // (i, j) 위치의 요소와 (i, N-1-j) 위치의 요소를 교환
	            int temp = matrix[i][j];
	...
	private static void rotation180(int[][] matrix) {
	    int N = matrix.length;
	    for (int i = 0; i < N / 2; i++) {
	        for (int j = 0; j < N; j++) {
	            // (i, j) 위치의 요소와 (N-1-i, N-1-j) 위치의 요소를 교환
	            int temp = matrix[i][j];
