...
public class _valid_input3 {
	...
	public static int sentenceSum(String str) {
		...
		for (String word : str.split(" ")) {
			sum = Math.multiplyExact(sum, 100);
			 sum = Math.addExact(sum, wordSum(word) % 100); 
		}
		return sum;
	}
	...
	public static void main(String[] args) {
		...
		if (!validateInput2(data.str)) {
		 ...
		 data.arr[0] = sentenceSum("computer preferred bulk tourist biographies");
		    data.arr[1] = sentenceSum("worldwide resistance implemented magical viruses");
		    data.arr[2] = sentenceSum("theorem");
