...
// --- 노드를 검색 ---//
	public boolean search(SimpleObject2 obj, Comparator<? super SimpleObject2> c) {
		Node4 ptr = first.rlink; // 더미 노드의 다음 노드부터 시작
		while (ptr != first) {
			if (c.compare(ptr.data, obj) == 0) // 검색 성공
				return true;
...
// --- 전체 노드 표시 ---//
	public void show() {
		...
		while (ptr != first) {
			System.out.println(ptr.data); // 노드를 출력
			ptr = ptr.rlink; // 다음 노드를 선택
		}
...
// --- 올림차순으로 정렬이 되도록 insert ---//
	public void add(SimpleObject2 obj, Comparator<? super SimpleObject2> c) {
		Node4 ptr = first; // 검색 포인터
		Node4 temp = new Node4(obj); // 삽입할 노드

		while (true) {
			Node4 next = ptr.rlink;
			if (next == first || c.compare(next.data, obj) > 0) {
				temp.rlink = next;
				ptr.rlink = temp;
				temp.llink = ptr;
				next.llink = temp;
...
// --- list에 삭제할 데이터가 있으면 해당 노드를 삭제 ---//
	...
	public DoubledLinkedList2 merge(DoubledLinkedList2 lst2) {
		...
		Node4 ptr1 = this.first.rlink;
		Node4 ptr2 = lst2.first.rlink;

		while (ptr1 != this.first && ptr2 != lst2.first) {
			if (ptr1.compareNode(ptr2) < 0) {
				mergedList.add(ptr1.data, SimpleObject2.NO_ORDER);
				ptr1 = ptr1.rlink;
			} else {
				mergedList.add(ptr2.data, SimpleObject2.NO_ORDER);
				ptr2 = ptr2.rlink;
			}
		}

		while (ptr1 != this.first) {
			mergedList.add(ptr1.data, SimpleObject2.NO_ORDER);
			ptr1 = ptr1.rlink;
		}

		while (ptr2 != lst2.first) {
			mergedList.add(ptr2.data, SimpleObject2.NO_ORDER);
			ptr2 = ptr2.rlink;
		}
