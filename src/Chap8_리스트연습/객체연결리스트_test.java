...
import Chap3_검색알고리즘.Fruit객체배열이진탐색_Test;
...
class LinkedList2 {
	...
	public int Delete(SimpleObject element, Comparator<SimpleObject> cc) // delete the element
	{
		Node2 temp = first;
		Node2 prev = null;
		if (temp != null && cc.compare(temp.data, element) == 0) {
			first = temp.link;
			return 1;
		}

		while (temp != null && cc.compare(temp.data, element) != 0) {
			prev = temp;
			temp = temp.link;
		}
	...
	public void Add(SimpleObject element, Comparator<SimpleObject> cc) // 임의 값을 삽입할 때 리스트가 오름차순으로 정렬이 되도록 한다
	{
		...
		if (first == null || cc.compare(first.data, newNode.data) >= 0) {
			newNode.link = first;
			first = newNode;
		} else {
			Node2 current = first;
			while (current.link != null && cc.compare(current.link.data, newNode.data) < 0) {
