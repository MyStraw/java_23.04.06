...
public class _6월10일_앵무새2 {
	public static void main(String[] args) throws IOException {
		...
		while (!result.isEmpty()) { // 최종 말이 다 비어야 possible 이겠지?
			if (loop > count) { // while을 한번 돌때 최소 result에서 한글자씩은 빠져 나가야 하는데, 그렇지 않으면 앵무새 첫말에 빠져나갈게 없단거.
								// impossible 이겠지? 이거 없으면 무한루프. loop는 count 보다 절대 클수가없다
				break;
			}
			for (int i = 0; i < N; i++) {
				if (listOfque.get(i).isEmpty()) { // 앵무새 마리수(i)를 각 돌면서 empty이면(먼저 말 다 뱉은 앵무새 처리) continue를 한다. 이거 안해주면
													// 에러.
					continue;
				}
