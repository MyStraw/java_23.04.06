...
public class B번_합이K이하6 {
	...
	public static void main(String[] args) throws IOException {
		...
		for (Map.Entry<Long, List<Long>> entry : groupByA.entrySet()) {
			...
			Collections.sort(bValues, Collections.reverseOrder());
			for (long b : bValues) {
				pairs.add(new Pair(a, b));
			}
		}

		Collections.sort(pairs, Comparator.comparingLong(p -> p.b));
		...
		while (left < right) {
			...
			if (leftPair.a == rightPair.a) { // 같은 a값이면 건너뛰기
