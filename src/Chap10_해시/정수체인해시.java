...
class Node {//노드 클래스 정의 (각 한칸)
    ...
    //--- 키의 해시값을 반환 ---//
    public int hashCode() { //해시값 임의값 자기가 만들어도 돼. 책처럼 개수를 나눠도 되고.
    	int hash = 11;
    	hash = 31* hash * key;
    	hash = hash * hash;
        return hash;
...
class SimpleChainHash { //전체 해시. 사이즈
 private int    size;              // 해시 테이블의 크기
 private Node[] table;        // 해시 테이블. [10]개. 

 //--- 생성자(constructor) ---//
 public SimpleChainHash(int capacity) { //노드 객체 만들어서 테이블 가리키게?
     try {
 ...
 //--- 해시값을 구함 ---//
 public int hashValue(int key) {//으잉 중복
 	int hash = 11;
 	hash = 31* hash * key;
 	hash = hash * hash;
     return hash;
 ...
 public int search(int key) { //사이즈 10개로 했으니 10으로 나눴다.
